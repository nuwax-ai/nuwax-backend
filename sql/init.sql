...
-- 修改表: publish_apply
ALTER TABLE `publish_apply`
    ADD COLUMN `ext` JSON COMMENT '扩展字段';

-- 修改表: custom_page_config
ALTER TABLE `custom_page_config`
    ADD COLUMN `sandbox_id` BIGINT COMMENT '沙盒ID';

-- 修改表: model_config
ALTER TABLE `model_config`
    ADD COLUMN `pid` VARCHAR(64) NOT NULL DEFAULT 'custom' COMMENT '提供商ID';
ALTER TABLE `model_config`
    ADD COLUMN `types` JSON COMMENT '模型能力类型列表（Text/Image/Audio/Video/TextEmbedding/MultiEmbedding/Reasoning）';

-- 修改表: custom_page_conversation
ALTER TABLE `custom_page_conversation`
    ADD COLUMN `session_id` VARCHAR(64) COMMENT '会话ID';
ALTER TABLE `custom_page_conversation`
    ADD COLUMN `role` VARCHAR(32) COMMENT '消息发送者角色';
ALTER TABLE `custom_page_conversation`
    ADD COLUMN `request_id` VARCHAR(64) COMMENT '请求ID';
...
-- 修改表: sandbox_config
ALTER TABLE `sandbox_config`
    ADD COLUMN `type` VARCHAR(16) NOT NULL DEFAULT 'Agent' COMMENT '沙箱类型：Agent 智能体沙箱；PageApp 应用开发沙箱';
ALTER TABLE `sandbox_config`
    ADD COLUMN `bind_info` JSON COMMENT '关系绑定';
ALTER TABLE `sandbox_config`
    ADD COLUMN `isolation` VARCHAR(16) COMMENT '隔离策略，仅页面开发有效';
...
ALTER TABLE `published`
    ADD FULLTEXT KEY `ft_name_desc` (`name`, `description`) WITH PARSER ngram;
