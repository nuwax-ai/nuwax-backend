...
@Builder
@Schema(name = "创建定时任务信息")
@AllArgsConstructor
@NoArgsConstructor
@Data
public class SandboxTaskAddDto {

    @Schema(description = "定时任务执行类型，Once 一次，需指定执行时间；Loop 循环，需指定执行周期cron表达式", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "定时任务执行时间，例如 2026-10-22T00:00:00，固定周期的可以不传")
