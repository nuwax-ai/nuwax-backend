...
import com.xspaceagi.agent.core.infra.component.model.anthropic.api.StreamHelper;
import reactor.core.publisher.Flux;
import reactor.core.publisher.Mono;

import com.xspaceagi.agent.core.infra.component.model.anthropic.api.AnthropicApi.ChatCompletionRequest.CacheControl;
import com.xspaceagi.agent.core.infra.component.model.anthropic.api.StreamHelper.ChatCompletionResponseBuilder;
...
public final class AnthropicApi {
	...
	public Flux<ChatCompletionResponse> chatCompletionStream(ChatCompletionRequest chatRequest,
		...
		AtomicInteger openToolBlocks = new AtomicInteger(0);
		...
		return this.webClient.post()
			...
			.map(event -> {
				...
				if (this.streamHelper.isToolUseStart(event)) {
					...
					openToolBlocks.incrementAndGet();
				}
				return event;
			})
			...
			.windowUntil(event -> {
				if (isInsideTool.get() && this.streamHelper.isToolUseFinish(event)) {
					int remaining = openToolBlocks.decrementAndGet();
					if (remaining <= 0) {
	...
	// @formatter:off
		...
		// cache object
		@JsonProperty("cache_control") ChatCompletionRequest.CacheControl cacheControl,
		...
		public ContentBlock(String text, ChatCompletionRequest.CacheControl cache) {
		...
		public ContentBlock(Source source, String title, String context, boolean citationsEnabled,
				ChatCompletionRequest.CacheControl cacheControl) {
		...
		public enum Type {
			 ...
			 * {@link com.xspaceagi.agent.core.infra.component.model.anthropic.AnthropicSkillsResponseHelper} to
		...
		public static class ContentBlockBuilder {
			...
			public ContentBlockBuilder cacheControl(ChatCompletionRequest.CacheControl cacheControl) {
