...
public class OpenAiApi {
    ...
    public Flux<ChatCompletionChunk> chatCompletionStream(ChatCompletionRequest chatRequest) {
        ...
        return chatCompletionStream(chatRequest, new LinkedMultiValueMap<>(), isStreamingCall);
    }
     ...
     */
    public Flux<ChatCompletionChunk> chatCompletionStream(ChatCompletionRequest chatRequest,
                                                          MultiValueMap<String, String> additionalHttpHeader, AtomicBoolean isStreamingCall) {

        Assert.notNull(chatRequest, "The request body can not be null.");
        Assert.isTrue(chatRequest.stream(), "Request must set the stream property to true.");
        chatRequest = chatRequest.thinking(isReasonModel);
        ...
        return this.webClient.post()
                ...
                .map(chunk -> {
                    ...
                    if (chunk != null && !CollectionUtils.isEmpty(chunk.choices()) && chunk.choices().get(0).delta() != null
                            && !CollectionUtils.isEmpty(chunk.choices().get(0).delta().toolCalls()) && chunk.choices().get(0).finishReason() == null) {
                        isStreamingCall.set(true);
                    }
                    return chunk;
                })
    ...
    public enum ChatCompletionFinishReason {
        ...
        /**
         * The model generated sensitive content.
         */
        @JsonProperty("sensitive")
        SENSITIVE;

        public static ChatCompletionFinishReason value(Object value) {
            if (value == null) {
                return null;
            }
            for (ChatCompletionFinishReason reason : values()) {
                if (reason.name().equals(value.toString().toUpperCase())) {
                    return reason;
                }
            }
            return null;
    ...
    public record ChatCompletionRequest(// @formatter:off
                                        ...
                                        @JsonProperty("thinking") Object thinking,
        ...
        public ChatCompletionRequest streamOptions(StreamOptions streamOptions) {
            return new ChatCompletionRequest(this.messages, this.model, this.store, this.metadata, this.frequencyPenalty, this.logitBias, this.logprobs,
                    ...
                    this.tools, this.toolChoice, this.parallelToolCalls, this.user, this.reasoningEffort, this.thinking, this.enableThinking ? true: null);
        }

        public ChatCompletionRequest thinking(boolean enableThinking) {
            //zhipu api
            Object thinking;
            if (enableThinking) {
            ...
            return new ChatCompletionRequest(this.messages, this.model, this.store, this.metadata, this.frequencyPenalty, this.logitBias, this.logprobs,
                    this.topLogprobs, this.maxTokens, this.maxCompletionTokens, this.n, this.outputModalities, this.audioParameters, this.presencePenalty,
                    this.responseFormat, this.seed, this.serviceTier, this.stop, this.stream, streamOptions, this.temperature, this.topP,
                    this.tools, this.toolChoice, this.parallelToolCalls, this.user, this.reasoningEffort, thinking, enableThinking? true: null);
        }
